export function Pagination({ currentPage, lastPage, onPageChange }) {
    ...
    return (
        <nav className="flex items-center justify-center gap-1 mt-8">
            <PageBtn onClick={() => onPageChange(currentPage - 1)} disabled={currentPage === 1}>← Əvvəlki</PageBtn>

            {pages.map((p, i) =>
                p === '...'
                    ? <span key={`e${i}`} className="px-2 text-gray-400 text-sm">…</span>
                    : <button key={p} onClick={() => onPageChange(p)}
                        className={`w-9 h-9 rounded-lg text-sm font-medium transition-colors ${p === currentPage ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-100'}`}>
                        {p}
                      </button>
            )}

            <PageBtn onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === lastPage}>Növbəti →</PageBtn>
        </nav>
    )
}
